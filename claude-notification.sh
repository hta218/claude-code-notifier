...
# This script displays system notifications when Claude Code triggers notification hooks
...
case "$(uname -s)" in
    Darwin*)
        ...
        if command -v terminal-notifier >/dev/null 2>&1; then
            terminal-notifier -title "Claude Code" -message "$message" -subtitle "Session: $session_id"
        else
            osascript -e "display notification \"$message\" with title \"Claude Code\" subtitle \"Session: $session_id\""
        fi
        ;;
    Linux*)
...
# Optional: Log notification to a file
# echo "$(date): Claude Code - $message" >> ~/.claude-notifications.log